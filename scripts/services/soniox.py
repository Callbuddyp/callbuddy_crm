...
def build_soniox_session(api_key: Optional[str] = None, env_path: Optional[str | Path] = None) -> Session:
    api_key = api_key or os.environ.get("SONIOX_API_KEY") or load_env_value("SONIOX_API_KEY", env_path)
    if not api_key:
        raise RuntimeError(
...
def get_config(
    ...
    config = {
        ...
        "language_hints": language_hints or ["da"],
        #
    ...
    if translation and translation != "none":
        # Translation config is optional; keep backward compatible defaults.
...
def transcribe_audio(
    ...
    keep_soniox: bool = False,
    translation: Optional[str] = None,
) -> "TranscriptionResult":
    file_id: Optional[str] = None
    transcription_id: Optional[str] = None
    try:
        file_id = upload_audio(session, str(audio_path))
        config = get_config(
            audio_url=None,
            file_id=file_id,
        ...
        transcription = get_transcription(session, transcription_id)
    finally:
        if not keep_soniox:
            if transcription_id:
                try:
...
def main():
    ...
    parser.add_argument(
        "--language_hints",
        nargs="*",
        help="Override Soniox language_hints (e.g. --language_hints da en).",
    )
    ...
    session = build_soniox_session(api_key)
    ...
    transcribe_file(
        session, args.audio_url, args.audio_path, args.translation, args.language_hints
    )
